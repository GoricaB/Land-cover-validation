...
def col_generator(size=5, chars=string.ascii_uppercase + string.digits):
    return ''.join(random.choice(chars) for _ in range(size))

random_name = 'a'+col_generator()


sites = processing.getObject(sites)

r_ext =sites.extent()
r_xmin = r_ext.xMinimum()
r_xmax = r_ext.xMaximum()
r_ymin = r_ext.yMinimum()
r_ymax = r_ext.yMaximum()

classified = processing.getObject(classified)

c_ext =classified.extent()
c_xmin = c_ext.xMinimum()
c_xmax = c_ext.xMaximum()
c_ymin = c_ext.yMinimum()
...
ref_ucat = list(set(list_ref))
all_ucat = sorted(list(set().union(ref_ucat,class_ucat)))

# Generate confusion matrix with numpy
np_string = ''
for item_c in all_ucat:
    for item_r in all_ucat:
        val_ok = list_combinations.count([item_r,item_c])
